...
const CardsContainer = (props) => {
    ...
    if (!isLoading && props.cards["Classic"]) {
        var cardSets = {
            "datas": [
                { name: "Classic", data: props.cards["Classic"].filter((card) => card.img) },
                { name: "Naxxramas", data: props.cards["Naxxramas"].filter((card) => card.img) },
                { name: "Hall of Fame", data: props.cards["Hall of Fame"].filter((card) => card.img) },
            ]
        }
